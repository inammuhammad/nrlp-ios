...
class AddBeneficiaryViewModelTests: XCTestCase {
    ...
    func testAddButtonPositive() {
        ...
        XCTAssertEqual(outputHandler.showAlert?.alertDescription, "Beneficiary created successfully, we have sent a SMS to the Beneficiary.")
...
class AddBeneficiaryViewModelOutputHandler {
    ...
    func setupObeserver() {
        viewModel.output = { output in
            ...
            case .nameTextField(let errorState, let errorMessage):
                self.nameError = errorMessage
            case .cnicTextField(let errorState, let errorMessage):
                self.cnicError = errorMessage
            case .mobileNumberTextField(let errorState, let errorMessage):
