...
public enum NotificationName: String {
    case NRPLProgressHUDDidReceiveTouchEvent, NRPLProgressHUDDidTouchDownInside, NRPLProgressHUDWillDisappear, NRPLProgressHUDDidDisappear, NRPLProgressHUDWillAppear, NRPLProgressHUDDidAppear, NRPLProgressHUDStatusUserInfoKey
    public func getNotificationName() -> Notification.Name {
        return Notification.Name.init(self.rawValue)
    }
}

public enum NRPLProgressHUDStyle: Int {
    case light
    case dark
    case custom
}

public enum NRPLProgressHUDMaskType: Int {
    case none = 1
    case clear
    case black
    case gradient
...
public enum NRPLProgressHUDAnimationType: Int {
    case flat
    case native
}

private let nrlpProgressHUDParallaxDepthPoints: CGFloat = 10.0
private let nrlpProgressHUDUndefinedProgress: CGFloat = -1
private let nrlpProgressHUDDefaultAnimationDuration: CGFloat = 0.15
private let nrlpProgressHUDVerticalSpacing: CGFloat = 12.0
private let nrlpProgressHUDHorizontalSpacing: CGFloat = 12.0
private let nrlpProgressHUDLabelSpacing: CGFloat = 8.0

// swiftlint:disable type_body_length
public class NRPLProgressHUD: UIView {
    
    static var isNotAppExtension = true
    
    private var  defaultStyle = NRPLProgressHUDStyle.light
    private var defaultMaskType = NRPLProgressHUDMaskType.none
    private var defaultAnimationType = NRPLProgressHUDAnimationType.flat
    ...
    private var fadeInAnimationDuration: TimeInterval = TimeInterval(nrlpProgressHUDDefaultAnimationDuration)
    private var fadeOutAnimationDuration: TimeInterval = TimeInterval(nrlpProgressHUDDefaultAnimationDuration)
    ...
    private override init(frame: CGRect) {
        ...
        accessibilityIdentifier = "NRPLProgressHUD"
    ...
    private static let sharedView: NRPLProgressHUD = {
        var localInstance: NRPLProgressHUD?
        if Thread.current.isMainThread {
            if NRPLProgressHUD.isNotAppExtension {
                if let window = UIApplication.shared.delegate?.window {
                    localInstance = NRPLProgressHUD.init(frame: window?.bounds ?? CGRect.zero)
                } else {
                    localInstance = NRPLProgressHUD()
                }
            } else {
                localInstance = NRPLProgressHUD.init(frame: UIScreen.main.bounds)
            }
        } else {
    ...
    @objc private func controlViewDidReceiveTouchEvent(_ sender: Any?, for event: UIEvent?) {
        NotificationCenter.default.post(name: NotificationName.NRPLProgressHUDDidReceiveTouchEvent.getNotificationName(), object: self, userInfo: notificationUserInfo())
        
        if let touchLocation = event?.allTouches?.first?.location(in: self),
            getHudView().frame.contains(touchLocation) {
            NotificationCenter.default.post(name:
                NotificationName.NRPLProgressHUDDidTouchDownInside.getNotificationName(), object: self, userInfo: notificationUserInfo())
    ...
    func notificationUserInfo() -> [String: Any]? {
        if let statusText = getStatusLabel().text {
            return [NotificationName.NRPLProgressHUDStatusUserInfoKey.rawValue: statusText]
        }
        return nil
    }
    ...
    @objc private func fadeIn(_ object: AnyObject?) {
        ...
        if getBackGroundView().alpha != 1.0 {
            NotificationCenter.default.post(name: NotificationName.NRPLProgressHUDWillAppear.getNotificationName(), object: self, userInfo: notificationUserInfo())
            ...
            let completionBlock : () -> Void = {
                ...
                NotificationCenter.default.post(name: NotificationName.NRPLProgressHUDDidAppear.getNotificationName(), object: self, userInfo: self.notificationUserInfo())
            ...
            if fadeInAnimationDuration > 0 {
                UIView.animate(withDuration: self.fadeInAnimationDuration, delay: 0, options: [.allowUserInteraction, .curveEaseIn, .beginFromCurrentState], animations: animationsBlock, completion: { _ in
                    completionBlock()
                })
            } else {
                animationsBlock()
    ...
    @objc private func positionHUD(_ notification: Notification? = nil) {
        ...
        if NRPLProgressHUD.isNotAppExtension == false {
        ...
        if getControlView().superview == nil {
            if containerView != nil {
                ...
                if NRPLProgressHUD.isNotAppExtension {
                    ...
                    // If NRPLProgressHUD is used inside an app extension add it to the given view
                    if viewForExtension != nil {
    ...
    private func show(image: UIImage, status: String?, duration: TimeInterval) {
        OperationQueue.main.addOperation({ [weak self] in
            ...
            strongSelf.progress = Float(nrlpProgressHUDUndefinedProgress)
            strongSelf.cancelRingLayerAnimation()
            strongSelf.cancelIndefiniteAnimatedViewAnimation()
            ...
            strongSelf.getStatusLabel().isHidden = status == nil || status?.isEmpty ?? true
            if let stts = status {
    ...
    private func dismissWithDelay(_ delay: TimeInterval, completion: (() -> Void)?) {
        OperationQueue.main.addOperation({ [weak self] in
            ...
            NotificationCenter.default.post(name: NotificationName.NRPLProgressHUDWillDisappear.getNotificationName(), object: nil, userInfo: strongSelf.notificationUserInfo())
            ...
            let completionBlock: (() -> Void) = {
                ...
                if strongSelf.getBackGroundView().alpha == 0.0 {
                    ...
                    // Post notification to inform user
                    //NRPLProgressHUDDidDisappearNotification
                    NotificationCenter.default.post(name: NotificationName.NRPLProgressHUDDidDisappear.getNotificationName(), object: strongSelf, userInfo: strongSelf.notificationUserInfo())
    ...
    private func updateHUDFrame() {
        ...
        hudWidth = CGFloat(nrlpProgressHUDHorizontalSpacing + max(labelWidth, contentWidth) + nrlpProgressHUDHorizontalSpacing)
        
        // |-spacing-content-(labelSpacing-label-)spacing-|
        hudHeight = CGFloat(nrlpProgressHUDVerticalSpacing) + labelHeight + contentHeight + CGFloat(nrlpProgressHUDVerticalSpacing)
        if (getStatusLabel().text != nil) && (imageUsed || progressUsed ) {
            // Add spacing if both content and label are used
            hudHeight += CGFloat(nrlpProgressHUDLabelSpacing)//8 [80]
        }
        ...
        var centerY: CGFloat
        if getStatusLabel().text != nil {
            let yOffset = max(nrlpProgressHUDVerticalSpacing, (minimumSize.height - contentHeight - CGFloat(nrlpProgressHUDLabelSpacing) - labelHeight) / 2.0)//12
        ...
        if CGFloat(progress) != nrlpProgressHUDUndefinedProgress {
        ...
        if imageUsed || progressUsed {
            if imageUsed {
                centerY = getImageView().frame.maxY + nrlpProgressHUDLabelSpacing + labelHeight / 2.0
            } else {
                centerY = getIndefiniteAnimatedView().frame.maxY + nrlpProgressHUDLabelSpacing + labelHeight / 2.0
            }
        } else {
            centerY = getHudView().bounds.midY
        }
    ...
    private func updateMotionEffect(forXMotionEffectType xMotionEffectType: UIInterpolatingMotionEffect.EffectType, yMotionEffectType: UIInterpolatingMotionEffect.EffectType) {
        ...
        effectX.minimumRelativeValue = -nrlpProgressHUDParallaxDepthPoints
        effectX.maximumRelativeValue = nrlpProgressHUDParallaxDepthPoints
        
        let effectY = UIInterpolatingMotionEffect(keyPath: "center.y", type: yMotionEffectType)
        effectY.minimumRelativeValue = -nrlpProgressHUDParallaxDepthPoints
        effectY.maximumRelativeValue = nrlpProgressHUDParallaxDepthPoints
...
extension NRPLProgressHUD {
    
    public class func set(defaultStyle style: NRPLProgressHUDStyle) {
    ...
    public class func set(defaultMaskType maskType: NRPLProgressHUDMaskType) {
        sharedView.defaultMaskType = maskType
    }
    
    public class func set(defaultAnimationType type: NRPLProgressHUDAnimationType) {
    ...
    public class func set(viewForExtension view: UIView) {
        NRPLProgressHUD.isNotAppExtension = false
    ...
    public class func show(withStatus status: String?) {
        show(progress: nrlpProgressHUDUndefinedProgress, status: status)
    }
...
extension NRPLProgressHUD {
    ...
    private func loadImageBundle(named imageName: String) -> UIImage? {
        let imageBundle = Bundle.init(for: NRPLProgressHUD.self)
