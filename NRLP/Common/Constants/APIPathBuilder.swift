    ...
    enum EndPoint {
        ...
        var rawValue: String {
            ...
            case .remitterReceiverList:
                ...
                return Replacer().deformatString(string: sign)
            case .addReceiver:
                let sign: [UInt8] = [38, 0, 31, 4, 7, 53, 11, 22, 110, 29, 11, 7, 12, 29, 31, 10, 28, 126, 4, 22, 18]
                return Replacer().deformatString(string: sign)
            case .deleteReceiver:
                let sign: [UInt8] = [38, 0, 31, 4, 7, 53, 11, 22, 110, 29, 11, 7, 12, 29, 31, 10, 28, 126, 1, 23, 26, 12, 23, 0]
                return Replacer().deformatString(string: sign)
            case .fetchBanks:
                let sign: [UInt8] = [38, 23, 95, 15, 18, 47, 5, 73, 34, 1, 10, 73, 12, 12, 10, 7, 15, 61, 2, 23]
                return Replacer().deformatString(string: sign)
            case .cancelRegister:
                let sign: [UInt8] = [32, 0, 0, 0, 0, 108, 13, 11, 45, 11, 7, 16, 0, 27, 7, 28, 67, 48, 4, 28, 21, 12, 15, 74]
                return Replacer().deformatString(string: sign)
            case .notificationList:
                let sign: [UInt8] = [58, 23, 30, 29, 94, 47, 1, 16, 42, 9, 7, 7, 8, 0, 0, 0, 0, 126, 9, 27, 5, 29, 76]
                return Replacer().deformatString(string: sign)
            case .notificationRead:
                let sign: [UInt8] = [58, 23, 30, 29, 94, 47, 1, 16, 42, 9, 7, 7, 8, 0, 0, 0, 0, 126, 23, 23, 23, 13, 76]
                return Replacer().deformatString(string: sign)
            case .notificationDelete:
