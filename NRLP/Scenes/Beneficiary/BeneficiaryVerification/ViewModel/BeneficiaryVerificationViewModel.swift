...
class BeneficiaryVerificationViewModel: BeneficiaryVerificationViewModelProtocol {
    ...
    private var service: APIKeyServiceDecorator<BeneficiaryVerificationServiceProtocol>
    ...
    init(service: APIKeyServiceDecorator<BeneficiaryVerificationServiceProtocol>,
    ...
    func nextButtonPressed() {
        ...
        service.dispatchForKey(cnic: model.cnicNicop, type: .beneficiary) {[weak self] (error) in
            ...
            self.service.decoratee.verifyCode(requestModel: VerifyRegistrationCodeRequestModel(nicNicop: self.model.cnicNicop, registrationCode: self.getVerificationCode(), userType: self.model.accountType)) {[weak self] (result) in
                guard let self = self else { return }
                self.output?(.showActivityIndicator(show: false))
                switch result {
                case .success:
