...
class AddBeneficiaryViewModel: AddBeneficiaryViewModelProtocol {
    ...
    func addButtonPressed() {
        ...
        service.addBeneficiary(beneficiary: AddBeneficiaryRequestModel(beneficiaryAlias: name, beneficiaryMobileNo: mobNumber, beneficiaryNicNicop: cnic, beneficiaryRelation: beneficiaryRelation, beneficiaryCountry: self.country?.country)) { [weak self] (result) in
            ...
            switch result {
            case .success:
                let model = AlertViewModel(alertHeadingImage: .successAlert, alertTitle: "Beneficiary Added".localized, alertDescription: "Beneficiary created successfully\nWe have sent an SMS to the Beneficiary with SDRP Link to download & Registration Code. Kindly ask your beneficiary to register within 3 days before the code expiry.".localized, primaryButton: AlertActionButtonModel(buttonTitle: "Done".localized, buttonAction: {
