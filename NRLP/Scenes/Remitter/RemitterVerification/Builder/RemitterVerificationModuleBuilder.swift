...
class RemitterVerificationModuleBuilder {
    ...
    func build(with navigationController: UINavigationController?, model: RegisterRequestModel) -> UIViewController {
        ...
        let viewModel = RemitterVerificationViewModel(service: APIKeyServiceDecorator(decoratee: RemitterVerificationService(), appKeyService: AppKeyService()), router: coordinator, model: model)
