...
//        self.navigationController?.pushViewController(, animated: true)
        if let nav = self.navigationController {
            ...
            let message = getSuccessMessage(psid: psid, amount: amount, flowType: flowType, inputModel: inputModel, transactionID: transactionID)
    ...
    private func getSuccessMessage(psid: String, amount: String, flowType: RedemptionFlowType, inputModel: InitRedemptionTransactionModel, transactionID: String) -> String {
        ...
        case .OPF:
            return "Your amount against Voucher No.\n\(psid) for \(amount) Points is redeemed successfully at OPF".localized
        case .DGIP:
            formatter.dateFormat = "dd MMM yyyy"
            let date = formatter.date(from: finalStrDate) ?? Date()
            let dateStr = formatter.string(from: date)
            return "Receipt No. \(transactionID)\nYou have redeemed \(amount) Points at Passport\n\(dateStr)".localized
        }
    }
}
