...
class ForgotPasswordViewModel: ForgotPasswordViewModelProtocol {
    ...
    init(router: ForgotPasswordRouter, service: APIKeyServiceDecorator<ForgotPasswordService>) {
        self.router = router
    ...
    private var service: APIKeyServiceDecorator<ForgotPasswordService>
    ...
    func nextButtonPressed() {
        ...
        service.dispatchForKey(cnic: cnic ?? "", type: accountType ?? .remitter) { [weak self] error in
            ...
            self.service.decoratee.sendOTP(requestModel: requestModel) {[weak self] (result) in
            self?.output?(.showActivityIndicator(show: false))
            switch result {
