...
extension NetworkManager: Networking {
    func get<T, R>(request: RequestBuilder<R>, completion: @escaping (APIResponse<T>) -> Void) -> APIRequest? where T: Decodable, R: Encodable {
        ...
        let task = HTTPTask.requestParametersAndHeaders(bodyParameters: nil, bodyEncoding: .urlAndJsonEncoding, urlParameters: try? JSONTranslation().toMap(withModel: request.parameters), additionHeaders: request.headers)
    ...
    func post<T, R>(request: RequestBuilder<R>, completion: @escaping (APIResponse<T>) -> Void) -> APIRequest? where T: Decodable, R: Encodable {
        ...
        let task = HTTPTask.requestParametersAndHeaders(bodyParameters: request.parameters, bodyEncoding: .jsonEncoding, urlParameters: nil, additionHeaders: request.headers)
